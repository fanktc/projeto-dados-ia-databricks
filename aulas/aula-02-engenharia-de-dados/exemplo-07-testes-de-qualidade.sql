...
SELECT * FROM (
    ...
    SELECT 1 AS n, 'receita preservada da bronze até a gold' AS teste,
           CAST((SELECT ROUND(SUM(receita),2) FROM rota_perfume.gold.fato_vendas) AS STRING) AS obtido,
           '102303828.05' AS esperado,
           (SELECT ROUND(SUM(receita),2) FROM rota_perfume.gold.fato_vendas) = 102303828.05 AS passou

    -- ── 2. Deduplicação funcionou ───────────────────────────────────────
    UNION ALL SELECT 2, 'CNPJ único na silver',
           CAST((SELECT COUNT(*) FROM (SELECT cnpj FROM rota_perfume.silver.clientes
                                       GROUP BY cnpj HAVING COUNT(*) > 1)) AS STRING),
           '0',
           (SELECT COUNT(*) FROM (SELECT cnpj FROM rota_perfume.silver.clientes
                                  GROUP BY cnpj HAVING COUNT(*) > 1)) = 0

    -- ── 3. Nenhuma data se perdeu na conversão ──────────────────────────
    UNION ALL SELECT 3, 'nenhuma data nula em pedidos',
           CAST((SELECT COUNT(*) FROM rota_perfume.silver.pedidos WHERE data_pedido IS NULL) AS STRING),
           '0',
           (SELECT COUNT(*) FROM rota_perfume.silver.pedidos WHERE data_pedido IS NULL) = 0

    -- ── 4. Receita negativa só pode existir em devolução ────────────────
    UNION ALL SELECT 4, 'receita negativa apenas em devolução',
           CAST((SELECT COUNT(*) FROM rota_perfume.gold.fato_vendas
                 WHERE receita < 0 AND NOT devolucao) AS STRING),
           '0',
           (SELECT COUNT(*) FROM rota_perfume.gold.fato_vendas
    ...
    UNION ALL SELECT 5, 'volume da fato_vendas entre 150k e 260k',
           CAST((SELECT COUNT(*) FROM rota_perfume.gold.fato_vendas) AS STRING),
           '150000 a 260000',
           (SELECT COUNT(*) FROM rota_perfume.gold.fato_vendas) BETWEEN 150000 AND 260000
    ...
    UNION ALL SELECT 6, 'nenhum pedido órfão no fato',
           CAST((SELECT COUNT(*) FROM (
                   SELECT DISTINCT f.pedido_id FROM rota_perfume.gold.fato_vendas f
                   LEFT ANTI JOIN rota_perfume.silver.pedidos p ON p.pedido_id = f.pedido_id
                 )) AS STRING),
           '0',
           (SELECT COUNT(*) FROM (
              SELECT DISTINCT f.pedido_id FROM rota_perfume.gold.fato_vendas f
    ...
    UNION ALL SELECT 7, 'nenhum cliente órfão no fato',
           CAST((SELECT COUNT(DISTINCT f.cliente_id) FROM rota_perfume.gold.fato_vendas f
                 LEFT ANTI JOIN rota_perfume.gold.dim_cliente c ON c.cliente_id = f.cliente_id) AS STRING),
           '0',
           (SELECT COUNT(DISTINCT f.cliente_id) FROM rota_perfume.gold.fato_vendas f
            LEFT ANTI JOIN rota_perfume.gold.dim_cliente c ON c.cliente_id = f.cliente_id) = 0

    -- ── 8. Os marts não podem divergir da fonte ─────────────────────────
    UNION ALL SELECT 8, 'mart de produto bate com o fato',
           CAST((SELECT ROUND(SUM(receita),2) FROM rota_perfume.gold.mart_produto_performance) AS STRING),
           '102303828.05',
           (SELECT ROUND(SUM(receita),2) FROM rota_perfume.gold.mart_produto_performance) = 102303828.05

    -- ── 9. CNPJ com 14 dígitos, zeros à esquerda preservados ────────────
    UNION ALL SELECT 9, 'todo CNPJ com 14 dígitos',
           CAST((SELECT COUNT(*) FROM rota_perfume.silver.clientes WHERE length(cnpj) <> 14) AS STRING),
           '0',
           (SELECT COUNT(*) FROM rota_perfume.silver.clientes WHERE length(cnpj) <> 14) = 0
) ORDER BY n;
