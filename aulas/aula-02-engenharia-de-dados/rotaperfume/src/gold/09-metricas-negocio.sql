...
-- marcas* e por *clientes em risco*. A view existe para que o nome da pergunta
-- e o nome da tabela sejam a mesma palavra — é isso que faz o agente acertar
-- de primeira em vez de tentar adivinhar qual JOIN fazer.
--
-- Repare que o COMMENT de cada view diz QUAL PERGUNTA ela responde, não o que
-- ela é. É assim que o Genie escolhe onde procurar.
...
SELECT f.categoria,
       ...
       ROUND(SUM(f.margem), 2)  AS margem,
       ROUND(SUM(f.margem) / NULLIF(SUM(f.receita), 0), 4) AS margem_pct,
       ROUND(AVG(p.margem_tabela_pct), 4) AS margem_tabela_pct,
...
CREATE OR REPLACE VIEW lakehouse_rotaperfume.gold.clientes_em_risco (
  ...
  dias_sem_comprar COMMENT 'Dias entre o último pedido do cliente e o último pedido da base inteira.',
...
SELECT c.cliente_id, c.razao_social, c.segmento, c.cidade,
       c.ultimo_pedido, c.dias_sem_comprar, c.total_pedidos,
